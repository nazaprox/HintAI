...
APP_NAME = "HintAI"
APP_VERSION = os.getenv("HINTAI_VERSION", "1.0.0")
ENVIRONMENT = os.getenv("ENVIRONMENT", os.getenv("VERCEL_ENV", "production"))
...
SUPPORTED_PLANS = tuple(PLAN_CREDITS)


def get_plan_credits(plan: str) -> int:
    return PLAN_CREDITS.get(str(plan).lower(), PLAN_CREDITS["free"])
...
def validate_configuration(require_gemini: bool = False) -> None:
    if not GEMINI_MODEL:
        raise RuntimeError("GEMINI_MODEL est obligatoire.")
    if require_gemini and not GEMINI_API_KEY:
        raise RuntimeError("GEMINI_API_KEY est obligatoire pour les fonctions IA.")
