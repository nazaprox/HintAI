...
function addHistory(label){ state.history.unshift({label,at:new Date().toLocaleString("fr-FR",{dateStyle:"short",timeStyle:"short"})}); state.history=state.history.slice(0,20); const box=$("history"); if(box) box.innerHTML=state.history.length?state.history.map(x=>`<div class="history-item"><span>✓</span><div><strong>${escapeHtml(x.label)}</strong><small>${x.at}</small></div></div>`).join(""):"Aucune activité."; saveState(); }
...
async function analyse(){
  const text=$("exerciseInput").value.trim(); const file=$("fileInput").files[0];
  ...
  try{
    ...
    if(file){payload.inputType=file.type==="application/pdf"?"pdf":"image"; const form=new FormData(); form.append("file",file); const upload=await fetch(`${API_BASE}/api/upload`,{method:"POST",body:form}); if(!upload.ok) throw new Error("Upload refusé"); const info=await upload.json(); payload.fileId=info.fileId;}
...
function init(){loadState();updateUI();initUpload();document.querySelectorAll("[data-page]").forEach(b=>b.addEventListener("click",()=>showScreen(b.dataset.page)));document.querySelectorAll("[data-go]").forEach(b=>b.addEventListener("click",()=>showScreen(b.dataset.go)));$("analyseBtn").onclick=analyse;$("hintBtn").onclick=getHint;$("solutionBtn").onclick=getSolution;$("learnStartBtn").onclick=startLearn);addHistoryFromState();}
function addHistoryFromState(){const box=$("history");if(box&&state.history.length)box.innerHTML=state.history.map(x=>`<div class="history-item"><span>✓</span><div><strong>${escapeHtml(x.label)}</strong><small>${x.at}</small></div></div>`).join("");}
